...
Rustation uses **Material UI (MUI) v7** as its exclusive design system. This ensures a consistent, professional, and accessible user interface that follows Google's Material Design 3 guidelines.

**Status**: ✅ Fully Migrated (Tailwind CSS and Shadcn UI have been removed)
...
2.  **Theming**: Define colors and shapes in `theme/index.ts`. Avoid hardcoded hex values.
3.  **Styled Components**: Use the `styled()` API for custom reusable components or complex styles.
    *   *Do not* write large inline `sx` objects.
    *   *Do not* use CSS files.
4.  **Sx Prop**: Use the `sx` prop only for simple, one-off layout adjustments (margins, padding).
