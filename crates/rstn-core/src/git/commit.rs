...
pub async fn intelligent_commit() -> Result<CommitResult> {
    ...
    tracing::debug!("Security scan complete: {} warnings, {} sensitive files, blocked={}",
        scan.warnings.len(), scan.sensitive_files.len(), scan.blocked);
    ...
    if files.trim().is_empty() {
        ...
        tracing::info!("Auto-staging successful, found {} files", files.lines().count());
    } else {
...
[
  {{
    ...
    tracing::info!("Arguments: -p \"{}\" --output-format text --max-turns 5",
        if prompt.len() > 50 { format!("{}...", &prompt[..50]) } else { prompt.clone() });
    tracing::info!("==========================");

    // Also output to console for user visibility
    eprintln!("$ claude -p \"<{} chars>\" --output-format text --max-turns 5", prompt.len());
    ...
    if !output.status.success() {
        ...
        tracing::error!("Claude command failed with exit code: {:?}", output.status.code());
        tracing::error!("stderr: {}", stderr);
        tracing::debug!("stdout: {}", stdout);
        return Err(crate::errors::CoreError::CommandFailed(
            format!("Claude command failed to analyze changes:\n{}", stderr.trim())
        ));
    }
    ...
    for (i, group) in groups.iter().enumerate() {
        tracing::debug!("Group {}: {} files - {}", i + 1, group.files.len(), group.message);
    }
...
async fn find_claude_executable() -> Result<String> {
    ...
    let home = std::env::var("HOME")
        .map_err(|_| crate::errors::CoreError::CommandFailed(
    ...
    Err(crate::errors::CoreError::CommandFailed(
        format!(
...
fn extract_json_from_response(response: &str) -> Result<String> {
    ...
    let preview = if response.len() > 200 {
        format!("{}... ({} more bytes)", &response[..200], response.len() - 200)
    } else {
        response.to_string()
    };

    tracing::error!("No JSON array found in Claude response. Response preview: {}", preview);
    Err(crate::errors::CoreError::Git(
        format!("No JSON array in response. Claude returned: {}", preview)
    ))
}
