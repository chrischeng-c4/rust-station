//! rstn-core - Pure Rust core library for rustation.
//!
...
/// Result type alias for rstn-core
...
// State update callback (for GPUI integration)
type StateCallback = Box<dyn Fn(&AppState) + Send + Sync>;
static STATE_CALLBACK: OnceLock<StateCallback> = OnceLock::new();

/// Get the global application state
pub fn get_app_state() -> &'static Arc<RwLock<AppState>> {
...
/// Push state update to registered callback
async fn notify_state_update() {
    if let Some(callback) = STATE_CALLBACK.get() {
        let state = get_app_state().read().await;
        callback(&*state);
    }
}

async fn get_docker_manager() -> Result<&'static Arc<DockerManager>> {
    DOCKER_MANAGER
...
pub async fn docker_list_services() -> Result<Vec<DockerService>> {
    let dm = get_docker_manager().await?;
    Ok(dm.list_services().await)
}

/// Start a Docker service
pub async fn docker_start_service(service_id: String) -> Result<()> {
    let dm = get_docker_manager().await?;
...
pub async fn docker_stop_service(service_id: String) -> Result<()> {
    let dm = get_docker_manager().await?;
...
pub async fn docker_restart_service(service_id: String) -> Result<()> {
    let dm = get_docker_manager().await?;
...
/// Get container logs
pub async fn docker_get_logs(service_id: String, tail: Option<u32>) -> Result<Vec<String>> {
    let dm = get_docker_manager().await?;
...
pub async fn docker_remove_service(service_id: String) -> Result<()> {
    let dm = get_docker_manager().await?;
...
pub async fn docker_create_database(service_id: String, db_name: String) -> Result<String> {
    let dm = get_docker_manager().await?;
...
pub async fn docker_create_vhost(service_id: String, vhost_name: String) -> Result<String> {
    let dm = get_docker_manager().await?;
...
pub async fn docker_start_service_with_port(service_id: String, port: u16) -> Result<()> {
    let dm = get_docker_manager().await?;
...
pub async fn docker_stop_container(container_id: String) -> Result<()> {
    let dm = get_docker_manager().await?;
...
pub async fn docker_check_port_conflict(service_id: String) -> Result<Option<state::PortConflictInfo>> {
    let dm = get_docker_manager().await?;
...
pub fn justfile_parse(path: String) -> Result<Vec<justfile::JustCommand>> {
    justfile::parse_justfile(&path)
        .map_err(|e| anyhow::anyhow!("{}", e))
}

/// Run a just command in a directory
pub fn justfile_run(command: String, cwd: String) -> Result<String> {
...
pub fn file_read(path: String, project_root: String) -> Result<String> {
...
/// - SECURITY_VIOLATION: Path outside allowed scope
/// - FILE_TOO_LARGE: File exceeds 10MB limit
pub fn file_read_binary(path: String, project_root: String) -> Result<Vec<u8>> {
...
pub fn worktree_list_branches(repo_path: String) -> Result<Vec<NapiBranchInfo>> {
...
pub async fn fetch_mcp_tools() -> Result<String> {
    // Check if state is initialized
    let state_cell = APP_STATE.get().ok_or_else(|| {
        anyhow::anyhow!("AppState not initialized. Call state_init first.")
    })?;
    let state: tokio::sync::RwLockReadGuard<'_, AppState> = state_cell.read().await;
...
/// Optionally provide a callback to be notified of state changes.
pub fn state_init(callback: Option<StateCallback>) -> Result<()> {
    ...
    // Register state callback if provided
    if let Some(cb) = callback {
        let _ = STATE_CALLBACK.set(cb);
    }
...
pub async fn state_get() -> Result<String> {
    let state = get_app_state().read().await;
...
pub async fn state_dispatch(action_json: String) -> Result<()> {
    // Parse the action
    let action: Action = serde_json::from_str(&action_json)
        .map_err(|e| anyhow::anyhow!("Invalid action JSON: {}", e))?;
...
async fn handle_async_action(action: Action) -> Result<()> {
    match action {
